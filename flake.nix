{
  ...
  inputs = {
    ...
    xrlinuxdriver = {
      url = "git+https://github.com/wheaney/XRLinuxDriver?ref=refs/pull/80/merge&submodules=1";
  ...
  outputs =
    ...
    in
    {
      packages = forEachSystem (
        pkgs:
        let
          version = pkgs.lib.trim (builtins.readFile "${self}/VERSION");
        in
        {
          ...
          breezy-desktop = pkgs.symlinkJoin {
            ...
            meta = {
              ...
              maintainers = with pkgs.lib.maintainers; [ shymega ];
              platforms = pkgs.lib.platforms.linux;
              mainProgram = self.packages.${pkgs.stdenv.hostPlatform.system}.breezy-ui.meta.mainProgram;
            };
          };
      ...
      nixosModules.default = import ./.nix/modules/breezy-desktop.nix { inherit self; };
    };
}
