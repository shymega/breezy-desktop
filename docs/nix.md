...
### Consuming this flake from elsewhere

The same tarball-vs-submodules limitation applies in the other direction: anyone
depending on breezy-desktop as a flake input must reference it via `git+https` with
`submodules=1` (e.g. `git+https://github.com/wheaney/breezy-desktop?submodules=1`), not
the `github:` shorthand — `self.submodules = true` only makes *this* flake fetch its own
submodules correctly when built directly, it does nothing for a downstream flake pinning
us via `github:`. See the [README's Nix section](../README.md#nix) for the exact input
syntax. This also rules out installing breezy-desktop as a plain GitHub tarball/archive
(`nix-env -f .../archive/main.tar.gz`) — that path never carries submodule content
either.
